...
def audit(N=64,L=6.0,radii=(0.75,1.0,1.25,1.5,2.0),nu=1.0):
    ...
    for ell in radii:
        ...
        int_v2=float(np.sum(v2[m])*dv)
        int_v3=float(np.sum(v2[m]**1.5)*dv)
        C=int_v2/ell
    ...
    return {'N':N,'L':L,'h':h,'max_divergence_grid':float(np.max(np.abs(divu))),'rows':rows}


def run_checks():
    a64=audit(64); a80=audit(80)
    r64={q['ell']:q for q in a64['rows']}; r80={q['ell']:q for q in a80['rows']}
    rel_rate_err=abs(r64[1.0]['ell2_dCdt']-r80[1.0]['ell2_dCdt'])/max(1.0,abs(r80[1.0]['ell2_dCdt']))
    checks={
        'mean_zero_64':max(q['mean_v_norm'] for q in a64['rows'])<1e-12,
        'mean_zero_80':max(q['mean_v_norm'] for q in a80['rows'])<1e-12,
        'positive_C_and_E':all(q['C_sph']>0 and q['E_sph']>0 for q in a64['rows']+a80['rows']),
        'finite_interpolation_ratio':all(np.isfinite(q['I3_over_CE_3_4']) for q in a64['rows']+a80['rows']),
        'nonzero_advective_channel_asymmetric':abs(r80[1.0]['A_adv'])>1e-4,
        'nonzero_pressure_channel_asymmetric':abs(r80[1.0]['P_pressure'])>1e-4,
        'rate_resolution_consistency_r1_below_3_percent':rel_rate_err<0.03,
    }
    return {
        ...
        'rate_r1_relative_difference_64_80':float(rel_rate_err),
...
def write_md(d,path):
    by={q['ell']:q for q in d['resolution_80']['rows']}
    lines=['# Mean-flow sphere oscillation budget','',f"Status: **{d['status']}**",'',f"Checks passed: **{d['passed']}/{d['total']}**",'',
           f"r=1 rate relative difference between 64^3 and 80^3: `{100*d['rate_r1_relative_difference_64_80']:.3f}%`.",'',
           '## Exact smooth-frame identity','',d['identity'],'',
           '## N=80 asymmetric benchmark','']
